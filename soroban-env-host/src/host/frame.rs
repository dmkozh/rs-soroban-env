...
///
/// With the storage model, storage rollback is handled by per-entry
/// stacks in the storage map. The frame depth is obtained from the Host
...
impl Host {
    ...
    pub(super) fn pop_context(&self, orp: Option<RollbackPoint>) -> Result<Context, HostError> {
        ...
        if let Some(rp) = orp {
            // Rollback: use commit_or_rollback_frame to pop stack entries at this depth
            self.try_borrow_storage_mut()?.commit_or_rollback_frame(
                true, // rollback = true
                self,
            )?;
            ...
            // Success: commit the frame's changes by decrementing depth
            self.try_borrow_storage_mut()?.commit_or_rollback_frame(
                false, // rollback = false (commit)
                self,
            )?;
        }
        ...
        F: FnOnce() -> Result<Val, HostError>,
    {
        ...
        // everything else.
        //
        // persist_instance_storage writes directly to storage via
        // modify_contract_instance.
    ...
    fn reload_instance_storage(&self, ctx: &mut Context) -> Result<(), HostError> {
        ...
        // Read from storage - it tracks values at each frame depth
        let instance = self.retrieve_contract_instance_from_storage(&instance_key)?;
