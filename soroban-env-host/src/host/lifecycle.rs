...
impl Host {
    ...
    pub(crate) fn upload_contract_wasm(&self, wasm: Vec<u8>) -> Result<BytesObject, HostError> {
        ...
        let code_key = Rc::metered_new(
            crate::storage::StorageKey::Other(crate::host::xdr_object::XdrObject::new(
                LedgerKey::ContractCode(LedgerKeyContractCode {
                    hash: Hash(hash_bytes.metered_clone(self.as_budget())?),
                }),
                36, // LedgerKey::ContractCode: 4 (disc) + 32 (hash)
            )),
            self.as_budget(),
        )?;
        ...
        if should_put_contract {
            ...
            storage.put(
                &code_key,
                &crate::storage::StorageEntry::LedgerEntry(Host::new_contract_code(self, data)?),
