...
impl InvocationResources {
    ...
    fn verify_limits(&self, host: &Host, limits: &InvocationResourceLimits) {
        ...
        if let Ok(storage) = host.try_borrow_storage() {
            let storage_iter = storage.map.iter();
            for (key, _entry) in storage_iter {
                ...
                if let Ok(maybe_entry) = storage.get_from_map(key, host) {
                    if let Some((entry, _live_until)) = maybe_entry {
                        // Reconstruct the LedgerEntry from the storage entry
                        if let Ok(ledger_entry) = entry.to_ledger_entry(key.as_ref(), host) {
                            let mut entry_buf = Vec::<u8>::new();
                            if metered_write_xdr(host.budget_ref(), &ledger_entry, &mut entry_buf)
                                .is_ok()
                            {
...
impl InvocationMeter {
    ...
    fn push_invocation<'a>(
        ...
        if self.stack_depth == 0 {
            ...
            // as the ledger state before invocation. Access types are part of
            // the storage entries and don't need separate reset.
...
impl Host {
    ...
    fn try_snapshot_storage_and_event_resources(
        ...
        let entries: Vec<_> = curr_storage
            .map
            .iter()
            .map(|(k, e)| (Rc::clone(k), e.access_type))
            .collect();
        for (key, curr_access_type) in entries {
            ...
            if let Some((init_entry, init_live_until_opt)) = maybe_init_entry {
                ...
                let init_ledger_entry = init_entry.to_ledger_entry(key.as_ref(), self)?;
                let mut buf = Vec::<u8>::new();
                metered_write_xdr(self.budget_ref(), &init_ledger_entry, &mut buf)?;
                if is_disk_read {
                    invocation_resources.disk_read_bytes += buf.len() as i32;
                }
                init_entry_size_for_rent =
                    entry_size_for_rent(self.budget_ref(), &init_ledger_entry, buf.len() as u32)?;
            }
