            ...
            if let Ok(footprint_iter) = storage.footprint.0.iter(host.budget_ref()) {
                for (key, _access_type) in footprint_iter {
                    ...
                    if let Ok(maybe_entry) = storage.get_from_map(key, host) {
                        if let Some(cached_entry) = maybe_entry {
                            // Reconstruct the LedgerEntry from the cached entry
                            if let Ok(entry) = cached_entry.to_ledger_entry(key.as_ref(), host) {
                                let mut entry_buf = Vec::<u8>::new();
                                if metered_write_xdr(host.budget_ref(), &entry, &mut entry_buf)
                                    .is_ok()
                                {
...
impl Host {
    ...
    fn try_snapshot_storage_and_event_resources(
        ...
        for (key, curr_access_type) in curr_footprint.0.iter(self.budget_ref())? {
            ...
            if let Some(init_cached_entry) = maybe_init_entry {
                let init_entry_live_until = init_cached_entry.live_until();
                ...
                let init_entry = init_cached_entry.to_ledger_entry(key.as_ref(), self)?;
                let mut buf = Vec::<u8>::new();
                metered_write_xdr(self.budget_ref(), &init_entry, &mut buf)?;
                if is_disk_read {
                ...
                event: ContractEvent {
                    ...
                    body: V0(
                        ContractEventV0 {
                            ...
                            data: String(
                                ScString(
                                    StringM(invocation resource limits are exceeded: instructions: 1876915 > 10, memory bytes: 1163086 > 20, contract events size bytes: 800 > 50, contract code entry with key 'ContractCode(LedgerKeyContractCode { hash: Hash(1a2ee5a89dd2162a20e716b3e2de70a2d662480a9565350378661871bcf8e398) })' size: 1840 > 45, contract data key 'ContractData(LedgerKeyContractData { contract: Contract(ContractId(Hash(2e0ff7a55065f3a896723a964c3d9862a4722bfc77229fe4875f390ef2a0027e))), key: LedgerKeyContractInstance, durability: Persistent })' size: 48 > 25, contract data entry with key 'ContractData(LedgerKeyContractData { contract: Contract(ContractId(Hash(2e0ff7a55065f3a896723a964c3d9862a4722bfc77229fe4875f390ef2a0027e))), key: LedgerKeyContractInstance, durability: Persistent })' size: 104 > 35),
                                ),
                            ),
                        },
                    ),
                ...
                event: ContractEvent {
                    ...
                    body: V0(
                        ContractEventV0 {
                            ...
                            data: String(
                                ScString(
                                    StringM(invocation resource limits are exceeded: instructions: 345830 > 10, memory bytes: 1134337 > 20, total footprint ledger entries: 5 > 4, disk read ledger entries: 2 > 1, disk read bytes: 3132 > 30, write ledger entries: 2 > 0, write bytes: 3132 > 40, contract code entry with key 'ContractCode(LedgerKeyContractCode { hash: Hash(fc644715caaead746e6145f4331ff75c427c965c20d2995a9942b01247515962) })' size: 3028 > 45, contract data key 'ContractData(LedgerKeyContractData { contract: Contract(ContractId(Hash(ba863dea340f907c97f640ecbe669125e9f8f3b63ed1f4ed0f30073b869e5441))), key: Symbol(ScSymbol(StringM(key_1))), durability: Persistent })' size: 60 > 25, contract data key 'ContractData(LedgerKeyContractData { contract: Contract(ContractId(Hash(ba863dea340f907c97f640ecbe669125e9f8f3b63ed1f4ed0f30073b869e5441))), key: LedgerKeyContractInstance, durability: Persistent })' size: 48 > 25, contract data entry with key 'ContractData(LedgerKeyContractData { contract: Contract(ContractId(Hash(ba863dea340f907c97f640ecbe669125e9f8f3b63ed1f4ed0f30073b869e5441))), key: LedgerKeyContractInstance, durability: Persistent })' size: 104 > 35),
                                ),
                            ),
                        },
                    ),
