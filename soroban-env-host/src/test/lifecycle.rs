...
mod cap_54_55_56 {
    ...
    use crate::{
        ...
        storage::{CachedEntry, FootprintMap, StorageMap},
        ...
        fn storage_map(&self, budget: &Budget) -> StorageMap {
            ...
            map.insert(
                ...
                Some(CachedEntry::Entry(
                    Rc::new(std::cell::RefCell::new((*self.contract_entry).clone())),
                    Some(99999),
                )),
                budget,
            )
            ...
            map.insert(
                self.wasm_key.clone(),
                Some(CachedEntry::Entry(
                    Rc::new(std::cell::RefCell::new((*self.wasm_entry).clone())),
                    Some(99999),
                )),
                budget,
            )
    ...
    fn test_v_new_call_wasm_in_footprint_but_not_storage() -> Result<(), HostError> {
        ...
        host.with_mut_storage(|storage| {
            let budget = host.budget_cloned();
            storage.map.insert(wasm_key, None, &budget)?;
            Ok(())
        })?;
    ...
    fn test_v_new_update_contract_with_module_cache() -> Result<(), HostError> {
        ...
        host.with_mut_storage(|storage| {
            ...
            storage.map.insert(wasm_code_key, None, host.as_budget())?;
            Ok(())
        })?;
