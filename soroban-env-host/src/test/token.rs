...
impl TokenTest {
    ...
    fn default_smart_token_with_admin_id(&self, admin: &ScAddress) -> TestToken {
    ...
    fn default_smart_token(&self, admin: &TestSigner) -> TestToken {
        self.default_smart_token_with_admin_id(&admin.address())
    }
...
fn test_smart_token_init_and_balance() {
    ...
    assert_eq!(
        to_contract_err(
            token
                .init(admin.address(), token_metadata.clone())
                .err()
                .unwrap()
        ),
    ...
    let token_id = Hash(token.id.to_vec().try_into().unwrap());

    let user = TestSigner::Ed25519(&test.user_key);

    assert_eq!(token.balance(user.address()).unwrap(), 0);
    assert_eq!(
        test.host.read_nonce(&token_id, &admin.address()).unwrap(),
        0
    );

    token.mint(&admin, user.address(), 10_000_000).unwrap();
...
fn test_smart_tokens_dont_support_classic_ops() {
    ...
    token.mint(&admin, user.address(), 100_000).unwrap();

    assert_eq!(
        to_contract_err(token.export(&user, 1).err().unwrap()),
        ContractError::OperationNotSupportedError
    );
    assert_eq!(
        to_contract_err(token.import(&user, 1).err().unwrap()),
        ContractError::OperationNotSupportedError
    );
}
...
fn test_native_token_smart_roundtrip() {
    ...
    assert_eq!(
        test.host
            .read_nonce(&expected_token_id, &user.address())
            .unwrap(),
        0
    );

    token.import(&user, 10_000_000).unwrap();
    ...
    token.export(&user, 5_000_000).unwrap();

    assert_eq!(test.get_native_balance(&account_id), 95_000_000);
    assert_eq!(token.balance(user.address()).unwrap(), 5_000_000);
    assert_eq!(
        test.host
            .read_nonce(&expected_token_id, &user.address())
            .unwrap(),
    ...
    assert_eq!(
        to_contract_err(token.import(&user, i64::MIN,).err().unwrap()),
    ...
    assert!(token.import(&user, i64::MAX,).is_err());
    assert!(token.export(&user, i64::MAX,).is_err());
}
...
fn test_classic_asset_roundtrip(asset_code: &[u8]) {
    ...
    token.import(&user, 1_000_000).unwrap();

    assert_eq!(
        test.get_classic_trustline_balance(&trustline_key),
        9_000_000
    );
    assert_eq!(token.balance(user.address()).unwrap(), 1_000_000);
    assert_eq!(
        test.host
            .read_nonce(&expected_token_id, &user.address())
            .unwrap(),
    ...
    token.export(&user, 500_000).unwrap();

    assert_eq!(
        test.get_classic_trustline_balance(&trustline_key),
        9_500_000
    );
    assert_eq!(token.balance(user.address()).unwrap(), 500_000);
    assert_eq!(
        test.host
            .read_nonce(&expected_token_id, &user.address())
            .unwrap(),
    ...
    assert_eq!(
        to_contract_err(token.export(&user, i64::MIN,).err().unwrap()),
        ContractError::NegativeAmountError
    );
...
fn test_transfer_with_allowance() {
    ...
    token.approve(&user, user_3.address(), 10_000_000).unwrap();
...
fn test_freeze_and_unfreeze() {
    ...
    assert!(!token.is_frozen(user.address()).unwrap());

    // Freeze the balance of `user`.
    token.freeze(&admin, user.address()).unwrap();
    ...
    token.unfreeze(&admin, user.address()).unwrap();
...
fn test_burn() {
    ...
    token.burn(&admin, user.address(), 40_000_000).unwrap();
    ...
    assert_eq!(
        to_contract_err(
            token
                .burn(&admin, user.address(), 60_000_001,)
                .err()
                .unwrap()
        ),
        ContractError::BalanceError
    ...
    token.burn(&admin, user.address(), 60_000_000).unwrap();
    assert_eq!(token.balance(user.address()).unwrap(), 0);
}
...
fn test_set_admin() {
    ...
    assert_eq!(
        to_contract_err(token.burn(&admin, new_admin.address(), 1).err().unwrap()),
        ContractError::UnauthorizedError
    );
    assert_eq!(
        to_contract_err(token.freeze(&admin, new_admin.address(),).err().unwrap()),
        ContractError::UnauthorizedError
    );
    assert_eq!(
        to_contract_err(token.unfreeze(&admin, new_admin.address(),).err().unwrap()),
        ContractError::UnauthorizedError
    );

    // The admin functions are now available to the new admin.
    token.mint(&new_admin, admin.address(), 1).unwrap();
    token.burn(&new_admin, admin.address(), 1).unwrap();
    token.freeze(&new_admin, admin.address()).unwrap();
    token.unfreeze(&new_admin, admin.address()).unwrap();
...
fn test_account_invoker_auth() {
    ...
    let token = test.default_smart_token_with_admin_id(&admin_address);
    ...
    assert_eq!(token.balance(admin_address.clone()).unwrap(), 2000);
...
fn test_classic_account_multisig_auth() {
    ...
    assert_eq!(
        to_contract_err(
            token
                .import(&TestSigner::account(&account_id, vec![&test.user_key]), 100,)
                .err()
                .unwrap()
        ),
    ...
    assert_eq!(
        to_contract_err(
            token
                .import(&TestSigner::account(&account_id, too_many_sigs,), 100,)
                .err()
                .unwrap()
        ),
        ContractError::AuthenticationError
...
fn test_negative_amounts_are_not_allowed() {
    ...
    assert_eq!(
        to_contract_err(token.burn(&admin, user.address(), -1,).err().unwrap()),
        ContractError::NegativeAmountError
    );
    ...
    assert_eq!(
        to_contract_err(token.approve(&user, user_2.address(), -1).err().unwrap()),
        ContractError::NegativeAmountError
    );

    // Approve some balance before doing the negative xfer_from.
    token.approve(&user, user_2.address(), 10_000).unwrap();
...
fn test_native_token_classic_balance_boundaries(
    ...
    assert_eq!(
        to_contract_err(
            token
                .import(&user, init_balance - expected_min_balance + 1,)
                .err()
                .unwrap()
        ),
        ContractError::BalanceError
    ...
    token
        .import(&user, init_balance - expected_min_balance)
        .unwrap();
    ...
    token
        .import(&large_balance_signer, i64::MAX - 10_000_000)
        .unwrap();
    token
        .xfer(
    ...
    assert_eq!(
        to_contract_err(
            token
                .export(&user, expected_max_balance - expected_min_balance + 1)
                .err()
                .unwrap()
        ),
        ContractError::BalanceError
    ...
    // possible.
    token
        .export(&user, expected_max_balance - expected_min_balance)
        .unwrap();
...
fn test_wrapped_asset_classic_balance_boundaries(
    ...
    token
        .mint(&issuer, user.address(), u64::MAX.into())
        .unwrap();
...
fn test_classic_transfers_not_possible_for_unauthorized_asset() {
    ...
    token.import(&user, 40_000_000).unwrap();
    assert_eq!(
        test.get_classic_trustline_balance(&trustline_key),
        60_000_000
    );
    ...
    // longer authorized.
    assert_eq!(
        to_contract_err(token.import(&user, 10_000_000,).err().unwrap()),
        ContractError::BalanceError
    );
    assert_eq!(
        to_contract_err(token.export(&user, 10_000_000,).err().unwrap()),
        ContractError::BalanceError
    );
}
