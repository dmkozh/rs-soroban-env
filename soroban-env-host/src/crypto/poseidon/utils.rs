...
pub(crate) fn sbox<S: MeteredScalar>(
    host: &Host,
    input: &Vec<S>,
    degree: usize,
...
pub(crate) fn add_rc<S: MeteredScalar>(
    host: &Host,
    input: &Vec<S>,
    rc: &Vec<S>,
) -> Result<Vec<S>, HostError> {
    if input.len() != rc.len() {
        return Err(host.err(
            ...
            "length mismatch between input array and round constants",
            &[
                Val::from_u32(input.len() as u32).into(),
                Val::from_u32(rc.len() as u32).into(),
            ],
        ));
    }
    input
