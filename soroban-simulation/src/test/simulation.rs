...
fn test_simulate_upload_wasm() {
    ...
    expect!["1754470"].assert_eq(&res.simulated_instructions.to_string());
    ...
    expect!["877235"].assert_eq(&res.simulated_memory.to_string());
    assert_eq!(
...
fn test_simulation_returns_insufficient_budget_error() {
    ...
    expect!["51806"].assert_eq(&res.simulated_memory.to_string());
...
fn test_simulate_create_contract() {
    ...
    assert!(res.contract_events.is_empty());
    assert!(res.diagnostic_events.is_empty());
    expect!["2814906"].assert_eq(&res.simulated_instructions.to_string());
    ...
    expect!["1407452"].assert_eq(&res.simulated_memory.to_string());
    assert_eq!(
        res.modified_entries,
        vec![LedgerEntryDiff {
            state_before: None,
