...
fn test_simulate_upload_wasm() {
    ...
    expect!["1757506"].assert_eq(&res.simulated_instructions.to_string());
    ...
    expect!["878753"].assert_eq(&res.simulated_memory.to_string());
    assert_eq!(
...
fn test_simulation_returns_logic_error() {
    ...
    assert_eq!(res.transaction_data, None);
    expect!["148712"].assert_eq(&res.simulated_instructions.to_string());
    expect!["74356"].assert_eq(&res.simulated_memory.to_string());
    assert_eq!(res.modified_entries, vec![]);
}
...
fn test_simulate_create_contract() {
    ...
    assert!(res.contract_events.is_empty());
    assert!(res.diagnostic_events.is_empty());
    expect!["2816074"].assert_eq(&res.simulated_instructions.to_string());
    ...
    expect!["1408036"].assert_eq(&res.simulated_memory.to_string());
    assert_eq!(
        res.modified_entries,
        vec![LedgerEntryDiff {
            state_before: None,
...
fn test_simulate_invoke_contract_with_auth() {
    ...
    expect!["41162756"].assert_eq(&res.simulated_instructions.to_string());
    ...
    expect!["20581360"].assert_eq(&res.simulated_memory.to_string());
    assert_eq!(
        res.modified_entries,
        vec![LedgerEntryDiff {
            state_before: None,
...
fn test_simulate_invoke_contract_with_autorestore() {
    ...
    let contract_1_size = contracts[1]
        ...
        .unwrap()
        .len() as u32;
    expect!["11183293"].assert_eq(&res.simulated_instructions.to_string());
    expect!["6231588"].assert_eq(
        &res.transaction_data
            .as_ref()
            .unwrap()
            .resource_fee
    ...
    expect!["5591638"].assert_eq(&res.simulated_memory.to_string());
    assert_eq!(
        res.modified_entries,
        vec![
            LedgerEntryDiff {
