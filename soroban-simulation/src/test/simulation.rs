...
fn test_simulate_upload_wasm() {
    ...
    expect!["1674087"].assert_eq(&res.simulated_instructions.to_string());
    ...
    expect!["837042"].assert_eq(&res.simulated_memory.to_string());
    assert_eq!(
...
fn test_simulation_returns_logic_error() {
    ...
    assert_eq!(res.transaction_data, None);
    expect!["154568"].assert_eq(&res.simulated_instructions.to_string());
    expect!["77284"].assert_eq(&res.simulated_memory.to_string());
    assert_eq!(res.modified_entries, vec![]);
}
...
fn test_simulate_create_contract() {
    ...
    assert!(res.contract_events.is_empty());
    assert!(res.diagnostic_events.is_empty());
    expect!["2742868"].assert_eq(&res.simulated_instructions.to_string());
    ...
    expect!["1371432"].assert_eq(&res.simulated_memory.to_string());
    assert_eq!(
        res.modified_entries,
        vec![LedgerEntryDiff {
            state_before: None,
...
fn test_simulate_invoke_contract_with_auth() {
    ...
    expect!["42090109"].assert_eq(&res.simulated_instructions.to_string());
    ...
    expect!["21045028"].assert_eq(&res.simulated_memory.to_string());
    assert_eq!(
        res.modified_entries,
        vec![LedgerEntryDiff {
            state_before: None,
...
fn test_simulate_invoke_contract_with_autorestore() {
    ...
    let contract_1_size = contracts[1]
        ...
        .unwrap()
        .len() as u32;
    expect!["10980502"].assert_eq(&res.simulated_instructions.to_string());
    expect!["6231385"].assert_eq(
        &res.transaction_data
            .as_ref()
            .unwrap()
            .resource_fee
    ...
    expect!["5490240"].assert_eq(&res.simulated_memory.to_string());
    assert_eq!(
        res.modified_entries,
        vec![
            LedgerEntryDiff {
